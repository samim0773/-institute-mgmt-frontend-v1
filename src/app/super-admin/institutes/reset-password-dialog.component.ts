...
export interface SuperAdminResetDialogData {
  userId:   string;
  userName: string;
...
@Component({
  selector: 'app-super-admin-reset-password-dialog',
  template: `
    <h2 mat-dialog-title>Reset Password</h2>
    <mat-dialog-content>
      <p class="rp-subtitle">
        Set a new password for <strong>{{ data.userName }}</strong>
        <span class="rp-role-badge" [class.rp-role-admin]="data.userRole === 'admin'">
          {{ data.userRole === 'admin' ? 'Admin' : 'Teacher' }}
        </span>
      </p>
    ...
    <style>
      .rp-subtitle { margin: 0 0 16px; color: #616161; font-size: 14px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
      .rp-role-badge { font-size: 11px; padding: 2px 8px; border-radius: 10px; background: #2e7d32; color: #fff; font-weight: 600; }
      .rp-role-badge.rp-role-admin { background: #1565c0; }
      .rp-form { display: flex; flex-direction: column; min-width: 340px; }
      .rp-field { width: 100%; margin-bottom: 8px; }
    </style>
  `,
})
export class SuperAdminResetPasswordDialogComponent {
  ...
  constructor(
    ...
    public  dialogRef: MatDialogRef<SuperAdminResetPasswordDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: SuperAdminResetDialogData,
  ) {
